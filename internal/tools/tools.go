...
func registerSetNodeProperty(server *mcp.Server, deps Deps) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Write. Sets a single property (string, integer, floating-point, boolean, " +
			...
			"AABB, Basis, Transform2D, Transform3D, or NodePath — exactly one of string_value/" +
			...
			"node_path_value must be given) on one node in a .tscn scene under the configured " +

...
func registerSetNodeProperty(server *mcp.Server, deps Deps) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Write. Sets a single property (string, integer, floating-point, boolean, " +
			"Vector2, Vector3, Color, Vector2i, Vector3i, Quaternion, Rect2, Rect2i, or Plane — " +
			"exactly one of string_value/int_value/float_value/bool_value/vector2_value/" +
			"vector3_value/color_value/vector2i_value/vector3i_value/quaternion_value/" +
			"rect2_value/rect2i_value/plane_value must be given) on one node in a .tscn scene " +

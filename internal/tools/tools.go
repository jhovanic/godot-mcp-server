...
func registerSetNodeProperty(server *mcp.Server, deps Deps) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Write. Sets a single property (string, integer, floating-point, boolean, " +
			...
			"AABB, Basis, Transform2D, Transform3D, NodePath, a PackedStringArray, or a " +
			"PackedInt32Array — exactly one of string_value/int_value/float_value/bool_value/" +
			...
			"int_array_value must be given) on one node in a .tscn scene under the configured " +

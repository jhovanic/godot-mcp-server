...
// Rect2 is a position + size rectangle, matching Godot's own Rect2 type.
// Used for properties like region_rect.
type Rect2 struct {
	Position Vector2 `json:"position" jsonschema:"the rectangle's position (top-left corner)"`
	Size     Vector2 `json:"size" jsonschema:"the rectangle's size (width and height)"`
}

// Rect2i is the integer-component counterpart to Rect2, matching Godot's
// own Rect2i type.
type Rect2i struct {
	Position Vector2i `json:"position" jsonschema:"the rectangle's position (top-left corner)"`
	Size     Vector2i `json:"size" jsonschema:"the rectangle's size (width and height)"`
}

// AABB is a 3D axis-aligned bounding box, matching Godot's own AABB type —
// the 3D counterpart to Rect2.
type AABB struct {
	Position Vector3 `json:"position" jsonschema:"the box's position (corner)"`
...
type Quaternion struct {
	X float64 `json:"x" jsonschema:"the quaternion's x component"`
	Y float64 `json:"y" jsonschema:"the quaternion's y component"`
	Z float64 `json:"z" jsonschema:"the quaternion's z component"`
...
// StringArrayValue, IntArrayValue, FloatArrayValue must be set — which one
...
// and PackedFloat32Array for now. Godot node properties also include other
// compound types (resource references, other packed array element types —
// Vector2, Vector3, Color, ...); supporting those is a separate, larger
...
type SetNodePropertyParams struct {
	...
	// IntArrayValue has the same nil-vs-explicitly-empty distinction as
	// StringArrayValue above, and for the same reason uses "omitzero" rather
	// than "omitempty" on its json tag.
	IntArrayValue []int64 `json:"int_array_value,omitzero" jsonschema:"set property_name to this PackedInt32Array value (e.g. SplitContainer.split_offsets); exactly one of the *_value fields must be set"`
	// FloatArrayValue has the same nil-vs-explicitly-empty distinction as
	// StringArrayValue/IntArrayValue above, and for the same reason uses
	// "omitzero" rather than "omitempty" on its json tag.
	FloatArrayValue []float64 `json:"float_array_value,omitzero" jsonschema:"set property_name to this PackedFloat32Array value (e.g. Label.tab_stops); exactly one of the *_value fields must be set"`
}
